...
export default function Purchased() {
  ...
  const [purchasedData, setPurchasedData] = useState<StockCars[] | Trucks[] | Machinery[]>([])

  useEffect(() => {
    const getData = async () => {
      switch (sort) {
        case 'Trucks':
          const TruckPurchased = await agent.LoadData.purchasedTrucksByCustomerID(user?.customerId);
          setPurchasedData(TruckPurchased.data)
          return
        case 'Machinery':
          const machinePurchased = await agent.LoadData.purchasedMachineryByCustomerID(user?.customerId);
          setPurchasedData(machinePurchased.data)
          return

        default:
          const carPurchased = await agent.LoadData.purchasedCarsByCustomerID(user?.customerId);
