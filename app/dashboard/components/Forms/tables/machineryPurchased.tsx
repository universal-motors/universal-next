...
export default function MachineryPurchasedTable() {
    const { user } = useUserStore();
    const [data, setData] = useState<Trucks[]>()
    useEffect(() => {
        const getData = async () => {
            const { data } = await agent.LoadData.purchasedTrucksByCustomerID(user.customerId);
            setData(data)
        };
        getData();
    }, [fetch]);
                ...
                {data?.map((item: Trucks) => {
                    return <tr className="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                        ...
                        <td className="px-6 py-4 flex">
                            <Link href={`/global/results/trucks/${item.stockId}`}>
