            ...
            return (
                <ul className="list-none p-0  overflow-y-auto max-w-250 border-2 border-gray-500 rounded-2xl">
                    {suggestions.map((suggestion: any, index: number) => {
                        ...
                        const classname = ` p-2 text-gray-700 cursor-pointer ${isSelected ? "bg-gray-200 text-black" : ""}`;
                        return (
                            <li
                                key={index}
                                className={classname}
...
export const Autocomplete = ({ list, placeholder, htmlFor, label }: Prop) => {
    ...
    return (
        <div>
            <Input value={inputValue} onChange={onChange} placeholder={placeholder} htmlFor={htmlFor} label={label} type={"type"} />
            <SuggestionsList
